...
(function(global){
  ...
  function buildContestConfig(contestDef){
    ...
    for(let i = 0; i < contestDef.numProblems; i++){
      ...
      // 题目难度计算
      // 检测比赛类型：正式比赛 vs 网赛 vs 台湾TOI比赛
      const isOnlineContest = contestDef.contestType === 'online';
      const isTaiwanContest = contestDef.taiwanOnly === true;
      const isOfficialContest = !isOnlineContest && !isTaiwanContest && typeof COMPETITION_DIFFICULTY_FACTORS !== 'undefined' && COMPETITION_DIFFICULTY_FACTORS[contestDef.name];
      
      let rawProblemDifficulty, problemDifficulty;
      
      // 如果是台湾TOI比赛
      if(isTaiwanContest && typeof TAIWAN_DIFFICULTY_FACTORS !== 'undefined'){
        const difficultyFactors = TAIWAN_DIFFICULTY_FACTORS[contestDef.name];
