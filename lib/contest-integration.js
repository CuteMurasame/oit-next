...
(function(global){
  ...
  function generateInternationalStudentsForTaiwan(taiwanStudents){
    ...
    for(let i = 0; i < mainlandCount; i++){
      ...
      const student = {
        name: `🇨🇳 中国大陆队-${mainlandNames[i] || '选手' + (i+1)}`,
  ...
  function distributeRewards(results, compName){
    ...
    for(let r of results){
      ...
      if(compName === 'NOI'){
        ...
        if(!r.passed) continue;
        reward = window.uniformInt ? 
          window.uniformInt(window.TOI_FINAL_REWARD_MIN || 25000, window.TOI_FINAL_REWARD_MAX || 40000) : 32000;
      } else if(compName === 'TOI复选'){
        // TOI复选：从28-30人选出10-12人
        if(!r.passed) continue;
        reward = window.uniformInt ? 
          window.uniformInt(window.TOI_RESELECTION_REWARD_MIN || 15000, window.TOI_RESELECTION_REWARD_MAX || 25000) : 20000;
      } else if(compName === 'TOI选训营'){
        // TOI选训营：选训营第一阶段
        if(!r.passed) continue;
        reward = window.uniformInt ? 
          window.uniformInt(window.TOI_TRAINING_REWARD_MIN || 8000, window.TOI_TRAINING_REWARD_MAX || 15000) : 11000;
      } else if(compName === 'TOI初选'){
        // TOI初选：从海选通过者中选出28-30人
        if(!r.passed) continue;
        reward = window.uniformInt ? 
          window.uniformInt(window.TOI_PRELIMINARY_REWARD_MIN || 4000, window.TOI_PRELIMINARY_REWARD_MAX || 8000) : 6000;
