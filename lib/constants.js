...
// 台湾 TOI 比赛链顺序
const TAIWAN_COMPETITION_ORDER = ["TOI海选","TOI初选","TOI选训营","TOI复选","TOI决选","IOI"];
...
/* =========== 台湾专用常量 =========== */
// 台湾使用 TOI 体系，不同于大陆的 CCF 体系
const TAIWAN_BUDGET = 180000;  // 台湾预算，略低于强省但高于普通省
const TAIWAN_TRAINING_QUALITY = 1.2;  // 训练质量高，接近强省水平
const TAIWAN_MIN_ABILITY = 45.0;  // 初始能力范围
const TAIWAN_MAX_ABILITY = 65.0;

// 台湾 TOI 比赛日程：海选→初选→选训营→复选→决选→IOI
const TAIWAN_COMPETITION_SCHEDULE = [
  {week:5, name:"TOI海选", difficulty:50, maxScore:100, numProblems:1, taiwanOnly:true},
  {week:12, name:"TOI初选", difficulty:120, maxScore:400, numProblems:4, taiwanOnly:true},
  {week:20, name:"TOI选训营", difficulty:280, maxScore:600, numProblems:6, taiwanOnly:true},  // 选训营第一阶段
  {week:22, name:"TOI复选", difficulty:380, maxScore:600, numProblems:6, taiwanOnly:true},    // 选训营第一阶段结束的复选
  {week:24, name:"TOI决选", difficulty:480, maxScore:800, numProblems:8, taiwanOnly:true},    // 选训营第二阶段的决选
  {week:28, name:"IOI", difficulty:650, maxScore:600, numProblems:6, taiwanOnly:true, subtasksPerProblem:15}
];

// 台湾比赛晋级线（基于历史数据：从2300-2800人筛选到最终4人）
// 已调低晋级线以降低难度
const TAIWAN_COMPETITION_CUTOFF = {
  "TOI海选": 0.10,     // 海选分数线10%，通过率相对较高
  "TOI初选": 0.30,     // 初选分数线30%，选出28-30人
  "TOI选训营": 0.50,   // 选训营分数线50%，第一阶段，为复选做准备
  "TOI复选": 0.58,     // 复选分数线58%，从28-30人筛到10-12人
  "TOI决选": 0.01,     // 决选分数线48%，从10-12人选出4人IOI国家队
  "IOI": 0.80          // IOI金牌线80%
};

// 台湾比赛难度系数
const TAIWAN_DIFFICULTY_FACTORS = {
  "TOI海选": [1.0],
  "TOI初选": [0.4, 0.8, 1.2, 1.7],
  "TOI选训营": [0.6, 0.8, 1.1, 1.4, 1.6, 1.7],
  "TOI复选": [0.7, 0.9, 1.1, 1.3, 1.5, 1.7],
  "TOI决选": [0.8, 1.0, 1.2, 1.4, 1.6, 1.8, 1.9, 2.0],
  "IOI": [0.8, 0.8, 1.2, 1.5, 1.8, 2.2]
};
...
const PROVINCES = {
 ...
 33:{name:"西藏",type:"弱省",isNorth:true,baseBudget:WEAK_PROVINCE_BUDGET,trainingQuality:WEAK_PROVINCE_TRAINING_QUALITY},
  ...
  // 台湾：特殊的 TOI 体系
 34:{name:"台湾",type:"台湾",isNorth:false,baseBudget:TAIWAN_BUDGET,trainingQuality:TAIWAN_TRAINING_QUALITY,isTaiwan:true}
};
...
if(typeof window !== 'undefined'){
  ...
  window.COMPETITION_ORDER = COMPETITION_ORDER;
  window.TAIWAN_COMPETITION_ORDER = TAIWAN_COMPETITION_ORDER;
}

// small export hint: keep variables in global scope (non-module script loaded before main script)
// Taiwan TOI system implementation complete
