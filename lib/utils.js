...
function getDailyChallengeParams()
{
  ...
  // 使用种子生成省份（1-34之间，包含台湾）
  const provinceId = Math.floor(seededRandom(seed) * 34) + 1;
...
let vis = new Array(names.length).fill(false);
...
function generateName(opts = {})
{
  ...
  {
    console.error('No more names can use');
    vis.fill(false);
  }

  let id = uniformInt(0, names.length - 1);
  for (; vis[id];)
  {
    id = uniformInt(0, names.length - 1);
  }
  vis[id] = true;
  return names[id];
}
