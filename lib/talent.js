  ...
  checkAndHandleTalentLoss(student) {
    ...
    registerDefaultTalents(game, utils){
      ...
      this.registerTalent({
        ...
        this.registerTalent({
          name: '原题机（真）',
          description: '在NOI系列比赛（不包括省选）中所有属性大大提升，但是有30%的概率被发现取消成绩并罚款以及降低声誉，这个概率在IOI中提升到60%且会影响其他选手成绩。',
          color: '#FF0000',
          prob: 0.02,
          kind: 'negative',
          beneficial: false,
            ...
            try{
              ...
              if(eventName === 'contest_start'){
                ...
                if(isNoiSeries){
                  ...
                  student.thinking = clamp(Number(student.thinking || 0) * 0.8);
                  student.coding = clamp(Number(student.coding || 0) * 0.8);
                  student.mental = clamp(Number(student.mental || 0) * 0.8);
                  student.knowledge_ds = Number(student.knowledge_ds || 0) * 0.8;
                  student.knowledge_graph = Number(student.knowledge_graph || 0) * 0.8;
                  student.knowledge_string = Number(student.knowledge_string || 0) * 0.8;
                  student.knowledge_math = Number(student.knowledge_math || 0) * 0.8;
                  student.knowledge_dp = Number(student.knowledge_dp || 0) * 0.8;
                  
                  student._talent_state['原题机（真）'] = { type: 'ioi' };
                  return '原题机（真）发动：IOI属性-20%';
                }
              }
