...
(function(global){
  const TalentManager = {
    ...
    // 注册单个天赋 (完整定义对象)
    // talentDef: { name, description, color?, prob?: 0.0-1.0, handler?: function, trainable?: boolean }
    registerTalent(talentDef){
      ...
      // trainable: 是否可以在集训中获得 (默认 true)
      const def = Object.assign({ description: '', color: '#2b6cb0', prob: 0.0, handler: null, kind: 'positive', trainable: true }, talentDef);
    ...
    handleStudentEvent(student, eventName, ctx){
      ...
      if(student.talents){
        // 将 Set 转换为数组并确保顺序一致性
        const talentArray = Array.from(student.talents);
        
        // 对于清理事件（比赛结束、模拟赛结束等），应该按相反顺序处理天赋
        // 这样可以确保依赖关系正确清理（后启用的天赋先禁用）
        const isCleanupEvent = eventName === 'contest_finish' || 
                              eventName === 'mock_contest_finish' || 
                              eventName === 'mock_end' ||
                              eventName === 'contest_cleanup';
        
        // 确保清理事件时按反向顺序处理：启用 ABCD，关闭 DCBA
        const talentsToProcess = isCleanupEvent ? [...talentArray].reverse() : talentArray;
        
        for(const tName of talentsToProcess){
  ...
  checkAndHandleTalentLoss(student) {
    ...
    // 激活人格担保天赋
    activatePersonalGuarantee(student){
      try{
        if(!student || !student.talents || !student.talents.has('人格担保')) return false;
        
        // 检查是否可以使用人格担保
        if(!student._talent_state || !student._talent_state._can_use_personal_guarantee) return false;
        
        // 移除原天赋，添加已人格担保天赋
        student.talents.delete('人格担保');
        student.talents.add('已人格担保');
        
        // 标记为已使用
        student._talent_state._人格担保_used = true;
        student._talent_state._can_use_personal_guarantee = false;
        
        // 记录事件
        const contestName = student._talent_state._personal_guarantee_contest || '某比赛';
        try{ 
          if(window.pushEvent){
            window.pushEvent({ 
              name: '人格担保发动', 
              description: `${student.name} 在${contestName}中使用了人格担保强行晋级，但之后所有比赛中属性将降低至70%！`, 
              week: window.game ? window.game.week : undefined 
            });
          }
        }catch(e){}
        ...
        console.error('activatePersonalGuarantee error', e);
        return false;
      }
    },
    ...
    registerDefaultTalents(game, utils){
      ...
      this.registerTalent({
        ...
        this.registerTalent({
          ...
          handler: function(student, eventName, ctx){
            try{
              ...
              // 在比赛进行中检查是否进入后半场
              if(eventName === 'contest_thinking' || eventName === 'contest_select_problem' || eventName === 'contest_solve_problem'){
                if(!ctx || !ctx.state) return null;
                const currentTick = ctx.state.tick || 0;
                const startTick = (student._talent_state && student._talent_state._慢热_startTick) || 0;
                const totalTicks = (student._talent_state && student._talent_state._慢热_totalTicks) || 180;
                const halfTick = startTick + Math.floor(totalTicks / 2);
                
                // 如果已进入后半场且尚未切换
                if(currentTick >= halfTick && student._talent_state && student._talent_state._慢热_phase === 'first'){
        ...
        // 人格担保：在NOI系列比赛（不包括IOI选拔）中如果没有晋级，可以选择发动天赋来晋级，但以后所有比赛中属性降低至70%
        this.registerTalent({
          name: '人格担保',
          description: '在NOI系列比赛中未晋级时，可以选择发动天赋强行晋级，但之后所有比赛中所有属性降低至70%。',
          color: '#4A90E2',
          prob: 0.03, // 可以随机获取
          kind: 'positive',
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              const contestName = ctx.contestName || '';
              const passed = ctx.passed || false;
              
              // 定义NOI系列比赛（不包括IOI选拔）
              const noiSeries = ['CSP-S1', 'CSP-S2', 'NOIP', 'NOI'];
              const isNoiSeries = noiSeries.includes(contestName);
              const isIOISelection = contestName === 'IOI选拔';
              
              // 只在NOI系列比赛且未晋级时触发
              if(!isNoiSeries || isIOISelection || passed) return null;
              
              // 检查是否已经使用过人格担保
              if(student._talent_state && student._talent_state._人格担保_used) return null;
              
              // 设置状态，让外部系统知道可以触发人格担保选择
              student._talent_state = student._talent_state || {};
              student._talent_state._can_use_personal_guarantee = true;
              student._talent_state._personal_guarantee_contest = contestName;
              
              return { 
                action: 'offer_personal_guarantee', 
                message: '人格担保：可以选择发动天赋强行晋级，但之后所有比赛中属性将降低至70%' 
              };
            }catch(e){ console.error('人格担保 天赋错误', e); }
            return null;
          }
        });

        // 已人格担保：人格担保使用后的状态，所有属性降低至70%
        this.registerTalent({
          name: '已人格担保',
          description: '已使用人格担保强行晋级，之后所有比赛中所有属性降低至70%。',
          color: '#8B4513',
          prob: 0.0, // 不能随机获取
          kind: 'negative',
          beneficial: false,
          trainable: false, // 不能在集训中获得
          handler: function(student, eventName, ctx){
            try{
              // 在比赛开始时应用属性惩罚
              if(eventName === 'contest_start' || eventName === 'mock_start'){
                // 使用临时增益系统：将所有属性降低30%（达到70%效果）
                const thinkingPenalty = -student._base_thinking * 0.3;
                const codingPenalty = -student._base_coding * 0.3;
                const mentalPenalty = -student._base_mental * 0.3;
                const dsPenalty = -student.knowledge_ds * 0.3;
                const graphPenalty = -student.knowledge_graph * 0.3;
                const stringPenalty = -student.knowledge_string * 0.3;
                const mathPenalty = -student.knowledge_math * 0.3;
                const dpPenalty = -student.knowledge_dp * 0.3;
                
                student.applyTempModifier('thinking', student.getTempModifier('thinking') + thinkingPenalty);
                student.applyTempModifier('coding', student.getTempModifier('coding') + codingPenalty);
                student.applyTempModifier('mental', student.getTempModifier('mental') + mentalPenalty);
                student.applyTempModifier('constmental', student.getTempModifier('constmental') + mentalPenalty);
                student.applyTempModifier('knowledge_ds', student.getTempModifier('knowledge_ds') + dsPenalty);
                student.applyTempModifier('knowledge_graph', student.getTempModifier('knowledge_graph') + graphPenalty);
                student.applyTempModifier('knowledge_string', student.getTempModifier('knowledge_string') + stringPenalty);
                student.applyTempModifier('knowledge_math', student.getTempModifier('knowledge_math') + mathPenalty);
                student.applyTempModifier('knowledge_dp', student.getTempModifier('knowledge_dp') + dpPenalty);
                
                return '已人格担保：所有属性降低至70%（本场临时）';
              }
            }catch(e){ console.error('已人格担保 天赋错误', e); }
            return null;
          }
        });
        ...
        // 原题机（真）：负面天赋，在NOI系列比赛（不包括省选）中属性大幅提升但有被发现风险
        this.registerTalent({
          name: '原题机（真）',
          description: '在NOI系列比赛（不包括省选）中所有属性大大提升，在IOI中属性一定下降。每次比赛有30%的概率被发现取消成绩并罚款以及降低声誉，这个概率在IOI中提升到60%且会影响其他选手成绩。',
          color: '#FF0000',
          prob: 0.02,
          kind: 'negative',
          beneficial: false,
          handler: function(student, eventName, ctx){
            try{
              ...
              if(eventName === 'contest_start'){
                const contestName = ctx && ctx.contestName;
                if(!contestName) return null;
                
                // 定义NOI系列比赛（不包括省选）
                const noiSeries = ['CSP-S1', 'CSP-S2', 'NOIP', 'NOI'];
                const isNoiSeries = noiSeries.includes(contestName);
                const isProvincial = contestName === '省选';
                const isIOI = contestName === 'IOI';
                
                // 省选中不生效
                if(isProvincial) return null;
                
                // 备份属性
                if(!student._talent_backup['原题机（真）']){
                  student._talent_backup['原题机（真）'] = {
                    ...
                    knowledge_graph: Number(student.knowledge_graph || 0),
                ...
                if(isNoiSeries){
                  // NOI系列：属性增加200%
                  student.thinking = clamp(Number(student.thinking || 0) * 3.0);
                  student.coding = clamp(Number(student.coding || 0) * 3.0);
                  student.mental = clamp(Number(student.mental || 0) * 3.0);
                  student.knowledge_ds = Number(student.knowledge_ds || 0) * 3.0;
                  student.knowledge_graph = Number(student.knowledge_graph || 0) * 3.0;
                  student.knowledge_string = Number(student.knowledge_string || 0) * 3.0;
                  student.knowledge_math = Number(student.knowledge_math || 0) * 3.0;
                  student.knowledge_dp = Number(student.knowledge_dp || 0) * 3.0;
                  
                  student._talent_state['原题机（真）'] = { type: 'noi_series' };
                  return '原题机（真）发动：NOI系列比赛属性+200%';
                } else if(isIOI){
                  // IOI：属性降低30%
                  student.thinking = clamp(Number(student.thinking || 0) * 0.7);
                  student.coding = clamp(Number(student.coding || 0) * 0.7);
                  student.mental = clamp(Number(student.mental || 0) * 0.7);
                  student.knowledge_ds = Number(student.knowledge_ds || 0) * 0.7;
                  student.knowledge_graph = Number(student.knowledge_graph || 0) * 0.7;
                  student.knowledge_string = Number(student.knowledge_string || 0) * 0.7;
                  student.knowledge_math = Number(student.knowledge_math || 0) * 0.7;
                  student.knowledge_dp = Number(student.knowledge_dp || 0) * 0.7;
                  
                  student._talent_state['原题机（真）'] = { type: 'ioi' };
                  return '原题机（真）发动：IOI属性-30%';
                }
              }
              
              // 比赛结束事件：检查是否被发现并应用惩罚
              if(eventName === 'contest_finish'){
                const contestName = ctx && ctx.contestName;
                if(!contestName) return null;
                ...
                // 检查发现概率
                let discoveryProb = 0;
                if(isNoiSeries) discoveryProb = 0.30;
                if(isIOI) discoveryProb = 0.60;
                
                if(getRandom() < discoveryProb){
                  // 被发现：成绩清零
                  const state = ctx && ctx.state;
                  if(state){
                    // 设置所有题目得分为0
                    ...
                    try{
                      if(typeof window !== 'undefined' && window.game){
                        const currentBudget = Number(window.game.budget || 0);
                        const fine = currentBudget * 0.5;
                        window.game.budget = Math.max(0, currentBudget - fine);
                        
                        // 声誉降低至20%（从当前声誉值降低80%）
                        const currentReputation = Number(window.game.reputation || 100);
                        window.game.reputation = Math.max(0, currentReputation * 0.2);
                        
                        // 记录事件
                        if(window.pushEvent){
                          window.pushEvent({
                            name: '原题机被发现',
                            description: `${student.name} 在${contestName}中被发现使用原题机！成绩清零，学校罚款${Math.round(fine)}元，声誉从${currentReputation}降至${Math.round(currentReputation * 0.2)}`,
                            week: window.game.week
                          });
                        }
                      }
                    }catch(e){ console.error('学校惩罚应用错误', e); }
                    
                    // IOI特殊处理：影响其他中国选手
                    if(isIOI){
                      const allStudents = ctx && ctx.allStudents;
                      if(Array.isArray(allStudents)){
                        // 找出所有中国选手（排除天赋持有者）
                        const chineseStudents = allStudents.filter(s => 
                          s !== student && s.nationality === '中国'
                        );
                        ...
                        if(chineseStudents.length > 0){
                          ...
                          for(let s of chineseStudents){
                            const score = s._contestState ? s._contestState.totalScore : 0;
                          ...
                          // 其他中国选手成绩减半（除最高分和天赋持有者）
                          for(let s of chineseStudents){
                            if(s !== highestStudent){
                              if(s._contestState){
                                s._contestState.totalScore = Math.floor(s._contestState.totalScore * 0.5);
                                // 更新所有题目得分
                                for(let prob of s._contestState.problems){
                                  prob.maxScore = Math.floor(prob.maxScore * 0.5);
                                }
                              }
                            }
                          }
                          
                          return `原题机（真）被发现：${student.name}成绩清零，其他中国选手除最高分外成绩减半`;
                        }
                      }
                    }
                    
                    return `原题机（真）被发现：${student.name}在${contestName}中成绩清零，学校罚款50%，声誉降至20%`;
                  }
                }
              }
              
              return null;
            }catch(e){ console.error('原题机（真） 天赋错误', e); }
            return null;
          }
        });
