...
class GameState {
  ...
  // 根据省份类型初始化晋级资格（台湾使用不同的比赛链）
  initQualificationForProvince(provinceType) {
    this.qualification = [ {}, {} ];
    const compOrder = (provinceType === "台湾" && typeof TAIWAN_COMPETITION_ORDER !== 'undefined') 
      ? TAIWAN_COMPETITION_ORDER 
      : COMPETITION_ORDER;
    for(let name of compOrder){ 
      this.qualification[0][name] = new Set(); 
      this.qualification[1][name] = new Set(); 
    }
  }
  getWeatherFactor(){
    let factor=1.0;
  ...
  getNextCompetition(){ const comps = (typeof window !== 'undefined' && window.competitions) || competitions; if(Array.isArray(comps) && comps.length > 0){ const sorted = comps.slice().sort((a, b) => a.week - b.week); const next = sorted.find(c => c.week > this.week); if(next){ let weeks_left = next.week - this.week; return next.name + ` (还有${weeks_left}周)`; } } return "无"; }
  updateWeather(){
    // New weather model:
...
/**
 * 根据省份类型初始化比赛日程
 * @param {string} provinceType - 省份类型（"台湾" 或其他）
 * @returns {Array} 比赛数组
 */
function initCompetitionsForProvince(provinceType) {
  if (provinceType === "台湾") {
    // 台湾使用 TOI 体系，按比例缩放到两个16周的半赛季
    const taiwanComps = [];
    if (typeof TAIWAN_COMPETITION_SCHEDULE !== 'undefined' && Array.isArray(TAIWAN_COMPETITION_SCHEDULE)) {
      // Taiwan schedule is designed for 32 weeks, scale to 16 weeks per half
      const originalTaiwanSeasonWeeks = 32;  // TAIWAN_COMPETITION_SCHEDULE的原始设计长度
      const firstHalfSize = WEEKS_PER_HALF;
      const secondHalfSize = SEASON_WEEKS - WEEKS_PER_HALF;
      
      // 第一年：将32周的Taiwan赛程按比例缩放到16周
      for (let comp of TAIWAN_COMPETITION_SCHEDULE) {
        const p = (comp.week - 1) / Math.max(1, (originalTaiwanSeasonWeeks - 1));
        let newWeek = 1 + Math.round(p * Math.max(0, firstHalfSize - 1));
        if (newWeek < 1) newWeek = 1;
        if (newWeek > firstHalfSize) newWeek = firstHalfSize;
        
        let copy = Object.assign({}, comp);
        copy.week = newWeek;
        if (!copy.numProblems) copy.numProblems = Math.max(1, Math.round((copy.maxScore || 400) / 100));
        if (!copy.maxScore) copy.maxScore = copy.numProblems * 100;
        taiwanComps.push(copy);
      }
    ...
    // 大陆省份使用默认的 competitions（已经初始化好的）
    return competitions;
  }
}
...
window.initCompetitionsForProvince = initCompetitionsForProvince;
